  ...
  (let ([state (lookup-state seating-chart coord)])
    (match state
      ...
      ['empty (if (= (count-occupied-neighbors seating-chart coord) 0)
                  (values 'occupied #t)
                  (values 'empty #f))]
      ['occupied (if (>= (count-occupied-neighbors seating-chart coord) 4)
                     (values 'empty #t)
                     (values 'occupied #f))])))
...
(define (surrounding-states seating-chart coord)
  ...
  (map (curry lookup-state seating-chart) (list (move coord -1 -1)
                                                (move coord 0 -1)
                                                (move coord 1 -1)
                                                (move coord 1 0)
                                                (move coord 1 1)
...
;; (void
;;   (begin
;;     (fill-seats-until-stable (parse-input "animation-input.txt") #:animate? #t))
;;     (display "\r"))
